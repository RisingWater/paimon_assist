...
| `src/tts_http.py` | HTTP TTS 后端（外部 EasyVoice API） |
| `src/tts.py` | TTS 后端分发（vits/http 动态切换） |
| `src/audio_manager.py` | 统一音频管理（播放队列 + 录音） |
...
| `src/llm_tools/` | 工具注册中心 + 天气/定位/搜索/空调/电视/记忆/提醒/音量 |
...
| `frontend/` | React + Vite + antd 前端（6 个 tab） |
...
`http://localhost:8160` — 八个栏目 + 系统配置弹窗：
...
│   ├── tts_api.py          # TTS API
│   ├── tts_cache.py        # TTS 缓存
│   ├── llm_tools/          # 工具注册 + 天气/定位/搜索/空调/电视/记忆/提醒/音量
