...
| `src/llm_tools/` | 工具注册中心 + 天气/定位/搜索/空调/电视/PS5/记忆/提醒/音量/门禁 |
...
Final 工具（`control_ac`/`control_tv`/`control_ps5`/`set_volume`/`open_door`/`add_reminder`/`delete_reminder`）：返回值直接作为最终 TTS 播出，跳过 LLM 二次加工，省 token 更快响应。
...
│   ├── tts_api.py          # TTS API
│   ├── tts_cache.py        # TTS 缓存
│   ├── llm_tools/          # 工具：天气/定位/搜索/空调/电视/PS5/记忆/提醒/音量/门禁
